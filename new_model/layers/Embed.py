...
class TokenEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.tokenConv(x)    # -> [batch_size, d_model, seq_len]
...
class TemporalEmbedding(nn.Module):
    ...
    def forward(self, x):
        x = x.long()
        minute_x = self.minute_embed(x[:, :, 5]) if hasattr(
...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.value_embedding = TokenEmbedding(c_in=c_in, d_model=d_model)  # c_in是传感器数量

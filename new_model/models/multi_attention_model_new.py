...
class MultiScaleSpatialAttention(nn.Module):
    ...
    def forward(self, x_dict, return_attention=True):
        # x_dict: {'cA{n}': [B, T1, N], 'cD{n}': [B, T2, N], ...}
        outputs = {}
        attention_weights = {}
        ...
        for k, x in x_dict.items():
            ...
            B, T, N = x.shape
            # 1. 升维
            x_proj = self.input_projection(x.unsqueeze(-1))  # [B, T, N, D]
            # 2. reshape for MHA over sensors
            x_flat = x_proj.reshape(B*T, N, -1)  # [B*T, N, D]
            residual = x_flat
            # 3. 空间注意力
            if return_attention:
                output, attn = self.transformer_encoder(x_flat, return_attention=True)
                attention_weights[k] = attn
            else:
                output = self.transformer_encoder(x_flat)
            # 4. 残差+归一化
            output = self.norm(output + residual)
            # 5. reshape回原始
            outputs[k] = output.reshape(B, T, N, -1)  # [B, T, N, D]
...
class MultiScaleTemporalAttention(nn.Module):
    ...
    def forward(self, x_dict, return_attention=True):
        # x_dict: {'cA{n}': [B, T1, N, D], 'cD{n}': [B, T2, N, D], ...}
        outputs = {}
        attention_weights = {}
        ...
        for k, x in x_dict.items():
            # x: [B, T, N, D]
            B, T, N, D = x.shape
            # 1. permute/reshape for MHA over time steps
            x_flat = x.permute(0, 2, 1, 3)  # [B, N, T, D]
            ...
            # 5. MHA
            if return_attention:
                output, attn = self.transformer_encoder(x_enc, return_attention=True)
                attention_weights[k] = attn
            else:
                output = self.transformer_encoder(x_enc)
            # 6. 残差连接+归一化
            output = self.norm(output + residual)
            # 7. reshape回原始
            outputs[k] = output.reshape(B, N, T, D).permute(0, 2, 1, 3)  # [B, T, N, D]
...
class WaveletBridgeFormer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.n_scales = args.n_scales if hasattr(args, 'n_scales') else 1  # 小波分解层数
        ...
        self.env_attention = EnvironmentalAttention(
            ...
            n_scales=self.num_wavelets_scales
        )
        
        # 初始化融合层
        self.fusion = FusionLayer(self.d_model, 1)
        
        print(f"模型初始化完成 - 传感器: {self.num_sensors}, 环境传感器: {self.num_env_sensors}, 小波尺度: {self.n_scales}")

    def get_wavelet_keys(self):
        """根据n_scales生成小波系数的键名列表"""
        keys = [f'cA{self.n_scales}']
        for i in range(self.n_scales, 0, -1):
            keys.append(f'cD{i}')
        return keys
    ...
    def forward(self, batch_x, batch_x_mark, decoder_input, batch_y_mark, mask=None, batch_env=None):
        ...
        # 检查输入的小波系数是否完整
        expected_keys = self.get_wavelet_keys()
        if not all(k in batch_x for k in expected_keys):
            raise ValueError(f"输入的小波系数不完整，需要包含以下系数：{expected_keys}")

        # 2. 空间注意力 - 分别处理每个尺度
        spatial_outputs, spatial_attns = self.spatial_attention(batch_x, return_attention=True)
        
        # 3. 时间注意力 - 分别处理每个尺度
        temporal_outputs, temporal_attns = self.temporal_attention(spatial_outputs, return_attention=True)
        ...
        for k in expected_keys:
            # 融合当前尺度的空间和时间特征
            curr_output = self.fusion(spatial_outputs[k], temporal_outputs[k])  # [B, T, N, D]
            # 降维
            curr_output = curr_output.squeeze(-1)  # [B, T, N]
            # 残差连接
            final_outputs[k] = curr_output + batch_x[k]
            # 保存中间状态
            hidden_states[f'fused_{k}'] = curr_output

        # 收集注意力权重
        attention_weights = {
            'spatial': spatial_attns,
            'temporal': temporal_attns,
        }

        return final_outputs, hidden_states, attention_weights
